...
if (credPath) {
  try {
    ...
    console.log(
      ...
      credPath,
      `(exists, ${stat.size} bytes)`
    );
  } catch (e) {
    console.warn(
      "GOOGLE_APPLICATION_CREDENTIALS is set but file not found or unreadable:",
      credPath,
      e.message
    );
  }
...
function createGoogleClient() {
  ...
  if (!cred) {
    console.warn(
      "Google credentials env var not set; will not create SpeechClient."
    );
    return null;
  }
  try {
  ...
  try {
    ...
    console.error(
      "Failed to create Google client. Ensure GOOGLE_APPLICATION_CREDENTIALS is set and valid.",
      err
    );
    return null;
  }
  ...
  if (!googleClient) {
    if (win && win.webContents)
      win.webContents.send(
        "status",
        "Google credentials missing — set GOOGLE_APPLICATION_CREDENTIALS"
      );
    return null;
  }
  ...
  const stream = googleClient
    ...
    .on("error", (err) => {
      ...
      if (err && err.code === "ENOTFOUND")
        console.warn("Network/ENOTFOUND while connecting to Google STT.");
    })
    .on("data", (data) => {
      if (data.results && data.results[0]) {
        ...
        if (win && win.webContents) {
          if (isFinal)
            win.webContents.send("transcript", { type: "final", text });
...
function startCapture(deviceIndex = null) {
  if (capturing) {
    ...
    if (win && win.webContents)
      win.webContents.send("status", "Already capturing");
    return;
  }
  ...
  if (deviceIndex === null) {
    if (win && win.webContents)
      win.webContents.send(
        "status",
        "No input device found. Open Settings and choose device."
      );
    return;
  }
  ...
  if (chosen && chosen.defaultSampleRate) {
    TARGET_SAMPLE_RATE =
      chosen.defaultSampleRate >= 48000
        ? 48000
        : Math.floor(chosen.defaultSampleRate);
  ...
  try {
    ...
    win.webContents.send(
      "status",
      "Failed to open audio input: " + err.message
    );
    if (recognizerStream) {
      try {
  ...
  ai.on("error", (err) => {
    ...
    if (win && win.webContents)
      win.webContents.send(
        "status",
        "Audio input error: " + (err && err.message ? err.message : String(err))
      );
  });
  ...
  try {
    ...
    if (win && win.webContents) {
      ...
      win.webContents.send("device-selected", {
        deviceId: deviceIndex,
        name: chosen ? chosen.name : String(deviceIndex),
      });
    }
    ...
    if (win && win.webContents)
      win.webContents.send(
        "status",
        "Failed to start capture: " + (e && e.message ? e.message : String(e))
      );
    try {
      ai.quit();
    } catch (ee) {}
